...
public class TabuSearch {
		...
		while (it < maxIter && System.currentTimeMillis() - t0 < maxTime) {
			...
			if (moves.size() <= 0) {
				...
				if(f1.getValue() >= best){
					nic++;
					if (nic > maxStable) {
						System.out.println(name()+ "::searchMaintainConstraintsFunctionMinimize --> restart.....");
						restartMaintainConstraintFunction(x, S, f2, tabu);
						nic = 0;
					}
				} else {
	...
	public void searchMaintainConstraintsMinimize(IFunction f, IConstraint S,
        ...
        public double searchMaintainConstraintsMinimize2(IFunction f, IConstraint S,
			int tabulen, int maxTime, int maxIter, int maxStable) {
		double t0 = System.currentTimeMillis();
			...
			if (moves.size() <= 0) {
				...
				if (f.getValue() < best) {
					...
					t_best = System.currentTimeMillis() - t0;
                                ...
                                if (((f.getValue() < best2)&&(S.violations()<=minViolation))||(S.violations()<minViolation)) {
					best2 = f.getValue();
					for (int i = 0; i < x.length; i++)
						x_best[i] = x[i].getValue();
                                        minViolation=S.violations();
					updateBest();
        ...
        public double searchMaintainConstraintsMinimize3(IFunction f, IConstraint S,
			int tabulen, int maxTime, int maxIter, int maxStable) {
		double t0 = System.currentTimeMillis();
                double t1= System.currentTimeMillis();;
		VarIntLS[] x = S.getVariables();
		...
		while (it < maxIter && System.currentTimeMillis() - t0 < maxTime) {
			...
			for (int i = 0; i < n; i++) {
				for (int v = x[i].getMinValue(); v <= x[i].getMaxValue(); v++) {
					...
					if (deltaS <= 0)
                        ...
                        for (int i = 0; i < n-1; i++) {
				for (int j= i+1; j< n; j++) {
                                        int deltaS = S.getSwapDelta(x[i], x[j]);
					...
					if (deltaS <= 0)
						if (tabu2[i][j] <= it
							...
							if (deltaF < minDelta) {
								...
								sel_v =j;
								moves.clear();
								moves.add(new TwoVariablesSwapMove(MoveType.TwoVariablesSwap, minDelta,  x[i], x[j]));
							} else if (deltaF == minDelta) {
								moves.add(new TwoVariablesSwapMove(MoveType.TwoVariablesSwap, minDelta,  x[i], x[j]));
							}
						}
			...
			if (moves.size() <= 0) {
				...
				Move m = moves.get(R.nextInt(moves.size()));
                                if(m instanceof OneVariableValueMove){
                                    sel_i = map.get(((OneVariableValueMove)m).getVariable());
                                
                                    sel_v = ((OneVariableValueMove)m).getValue();
                                    x[sel_i].setValuePropagate(sel_v);
                                    tabu[sel_i][sel_v - minV] = it + tabulen;
                                    System.out.println(name()
                                                    + "::searchMaintainConstraintsMinimize, Step " + it
                                ...
                                else if(m instanceof TwoVariablesSwapMove){
                                    sel_i = map.get(((TwoVariablesSwapMove)m).getVar1());
                                    sel_v = map.get(((TwoVariablesSwapMove)m).getVar2());
                                    int temp;
                                    temp=x[sel_i].getValue();
                                    x[sel_i].setValuePropagate(x[sel_v].getValue());
                                    x[sel_i].setValuePropagate(temp);
                                    tabu2[sel_i][sel_v] = it + tabulen;
                                    System.out.println(name()
                                                    + "::searchMaintainConstraintsMinimize, Step " + it
                                                    + ", S = " + S.violations() + ", f = " + f.getValue()
                                                    + ", best = " + best + ", delta = " + minDelta
