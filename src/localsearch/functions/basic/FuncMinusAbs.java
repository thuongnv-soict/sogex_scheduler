...
public class FuncMinusAbs extends AbstractInvariant implements IFunction {
	...
	public FuncMinusAbs(int val, IFunction f){
		_f1 = new FuncVarConst(f.getLocalSearchManager(),val);
		_f2 = f;
		_ls = f.getLocalSearchManager();
		post();
	}
	public FuncMinusAbs(IFunction f1, IFunction f2) {
		this._f1 = f1;
		this._f2 = f2;
	...
	public FuncMinusAbs(IFunction f, VarIntLS x) {
		_f1 = f;
	...
	public FuncMinusAbs(IFunction f, int val) {
		_f1 = f;
	...
	public FuncMinusAbs(VarIntLS x, VarIntLS y) {
	...
	public FuncMinusAbs(VarIntLS x, int val) {
		_f1 = new FuncVarConst(x);
	...
	private void post() {
		...
		_maxValue = _f1.getMaxValue() - _f2.getMinValue();
                if(_maxValue <0) _maxValue =-_maxValue ;
		_minValue = _f1.getMinValue() - _f2.getMaxValue();
                if(_minValue <0) _minValue =-_minValue ;
		_ls.post(this);
	}
	...
	public static void main(String[] args) {
		...
		FuncMinusAbs sum = new FuncMinusAbs(x,y);
		x.setValue(10);
		y.setValue(15);
		_ls.close();		
