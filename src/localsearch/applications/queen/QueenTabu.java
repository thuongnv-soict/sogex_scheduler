...
public class QueenTabu {
	 ...
	 */
         ...
         public void search(IConstraint c, int tbl){
             VarIntLS[] x=c.getVariables();
             int n=x.length;
             int max=0;
             for(int i=0;i<n;i++){
                 if(max< x[i].getMaxValue()) max=x[i].getMaxValue();
             }
             for(int i=0;i<n;i++){
                for(int v=0;v<n;v++){
                }
		...
		ts.search(S, n, 1000000, 100000, 200);
