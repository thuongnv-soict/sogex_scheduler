...
/**
 *
 * @author Pháº¡m Tiáº¿n DÅ©ng
 */


public class BCAgreedy {
    private static final String  fileInput="BCArandom.txt";
    private static final String  fileOutput="BCAgreedy.html";
    ...
    public boolean tabuSearch() {
        ...
        //while(S.violations()>0){
            for(int i=0;i<classes.size();i++) {
                x[i].setValuePropagate(Service.rand(0, instructors.size()-1));
                
            }
             ts.searchMaintainConstraintsMinimize2(difTime,S, 5000, 5000, 10000, 100);
        //}
    ...
    public void printSolutionHTML(String fn) {
		try {   
			...
			bw.write("<body><table BORDER=1 \"WIDTH:1920px\" BGCOLOR=\"white\" align=center><tr><th colspan=64>Thá»�i khÃ³a biá»ƒu</th></tr>");
			// in cac muc
			bw.write("<tr>");
			bw.write("<TH ROWSPAN =2 colspan=2 BGCOLOR=\"gray\" \"> Giang vien </ TH>");

			for (int i = 2; i < 7; i++) {
				bw.write("<th colspan=\"12\" STYLE=\"WIDTH:360px\" BGCOLOR=\"gray\">Thá»© "
						+ i + "</th>");
			}
			bw.write("</tr>");
			bw.write("<tr>");
			for (int i = 0; i < 60; i++) {
				bw.write("<td colspan=\"1\" STYLE=\"WIDTH:30px \" BGCOLOR=\"gray\">tiáº¿t "
						+ (i % 12 + 1) + "</td>");
			}
			...
			for (int i = 0; i < instructors.size(); i++) {
				// System.out.println("Lá»›p : " + classes.get(i));
				...
				for (int t = 0; t < 59; t++) {
					...
					for (int j = 0; j < classes.size(); j++) {
						...
						bw.write("<td STYLE=\"WIDTH:"+ weight+ "px\" BGCOLOR=\"lightgray\" colspan="+ weight+ ">"
                                                            + " Ma lop hoc:  "+ classes.get(j).classID+ " , "
                                                            + " Ma mon hoc:  "+ classes.get(j).courseiD+ " , "
                                                            + "Ten mon hoc:  "+ map.get(classes.get(j).courseiD).courseName +    "</td>");
    ...
    public static void main(String[] args) {
		...
		System.out.println(bca.classes.get(3).slot[0]);
                for(int i=0;i<bca.classes.size();i++) {
                    if(bca.x[i].getValue()==-1) {
...
class InstructorTime implements Comparable{
    public int instructorID;
    public ConditionalSum instructorTime;

    public InstructorTime(int instructorID, ConditionalSum instructorTime) {
        this.instructorID = instructorID;
        this.instructorTime = instructorTime;
    }
    @Override
    public int compareTo(Object o) {
        int compareage=((InstructorTime)o).instructorTime.getValue();
        /* For Ascending order*/
        return this.instructorTime.getValue()-compareage; //To change body of generated methods, choose Tools | Templates.
    }
}
